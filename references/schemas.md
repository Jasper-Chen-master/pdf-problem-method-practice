...
Use UTF-8 JSON. Each JSONL file contains exactly one JSON object per line. All generated files live under the target project's `problem_bank/` directory.
...
`solution_text` may be empty only when `provisional` is true. `question_id` must be unique and stable for the source path plus question number.
